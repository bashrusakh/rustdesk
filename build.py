...
def build_libdrmtap_so():
    ...
    # Verify the pin and cleanliness whenever the source is a GIT checkout. A fetch by sha cannot resolve to anything
    ...
    # cleanly. A NON-git or otherwise unverifiable tree placed here on purpose (a developer building
    # unreleased local libdrmtap source) may be used only with an explicit local opt-in.
    ...
    if os.path.exists(git_dir):
        try:
            ...
            dirty = subprocess.check_output(
                ['git', '-C', src, 'status', '--porcelain=v1', '--untracked-files=all'],
                stderr=subprocess.STDOUT,
            ).decode()
        except (subprocess.SubprocessError, OSError, UnicodeError):
            if not DRMTAP_UNPINNED_OK:
                raise Exception(
                    f'libdrmtap at {src} is an unverifiable or dirty git tree; '
                    'set DRMTAP_ALLOW_UNPINNED=1 for local source')
            print('WARNING: libdrmtap source tree is unverifiable or dirty; DRMTAP_ALLOW_UNPINNED=1 set')
        else:
            ...
            if dirty:
                if not DRMTAP_UNPINNED_OK:
                    raise Exception(
                        f'libdrmtap at {src} is a dirty checkout; '
                        'set DRMTAP_ALLOW_UNPINNED=1 for local source')
                print('WARNING: libdrmtap source tree is dirty; DRMTAP_ALLOW_UNPINNED=1 set')
    elif not DRMTAP_UNPINNED_OK:
        raise Exception(
            f'libdrmtap at {src} is a non-git source tree; '
            'set DRMTAP_ALLOW_UNPINNED=1 for local source')
    else:
        print('WARNING: libdrmtap source tree is non-git; DRMTAP_ALLOW_UNPINNED=1 set')
...
def build_deb_from_folder(version, binary_folder, want_drm=False):
    os.chdir('flutter')
    stage_custom_txt_for_linux_bundle(Path('..') / binary_folder)
    system2('mkdir -p tmpdeb/usr/bin/')
    system2('mkdir -p tmpdeb/usr/share/rustdesk')
    system2('mkdir -p tmpdeb/usr/share/rustdesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
